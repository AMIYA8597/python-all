# Vector Embeddings in the Modern LLM Era: A Comprehensive Textbook Reference
...
## 2. High-Dimensional Space: The Geometry of Meaning
...
## 3. The Rigorous Mathematics of Embedding Vectors
...
$$ \mathbf{v} = egin{bmatrix} v_1 \ v_2 \ v_3 \ dots \ v_d \end{bmatrix} \quad 	ext{where } v_i \in \mathbb{R} $$
...
$$ \mathbf{v}(	ext{"King"}) - \mathbf{v}(	ext{"Man"}) + \mathbf{v}(	ext{"Woman"}) pprox \mathbf{v}(	ext{"Queen"}) $$
...
## 4. Capturing Semantic Meaning: Training Objectives
...
$$ \mathcal{L} = -\log rac{\exp(	ext{sim}(\mathbf{e}_A, \mathbf{e}_P) / 	au)}{\exp(	ext{sim}(\mathbf{e}_A, \mathbf{e}_P) / 	au) + \sum_{j=1}^{k} \exp(	ext{sim}(\mathbf{e}_A, \mathbf{e}_{N_j}) / 	au)} $$
...
## 5. Measuring Similarity: The Mathematics of Cosine Similarity
...
$$ 	ext{Cosine Similarity}(\mathbf{u}, \mathbf{v}) = \cos(	heta) = rac{\mathbf{u} \cdot \mathbf{v}}{\|\mathbf{u}\|_2 \|\mathbf{v}\|_2} $$
...
## 6. The Curse of Dimensionality and Vector Databases
...
$$ \lim_{d 	o \infty} rac{d_{max} - d_{min}}{d_{min}} 	o 0 $$
...
$$ V_d(R) = rac{\pi^{d/2}}{\Gamma(rac{d}{2} + 1)} R^d $$
...
## 7. Applied Python: Working with Embeddings in Code
