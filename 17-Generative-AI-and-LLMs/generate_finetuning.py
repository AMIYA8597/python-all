...
markdown_content = """# Generative AI and LLMs: A Comprehensive Guide to Fine-Tuning
...
$$ \text{obj}(\theta) = \mathbb{E}_{(x, y) \sim \pi_\theta} \left[ r_\phi(x, y) - \beta \mathbb{D}_{KL}(\pi_\theta(y|x) \parallel \pi_{SFT}(y|x)) \right] $$
...
*   $\beta$ is a coefficient (often dynamic) that controls the strength of this KL penalty.
...
LoRA also introduces a scaling factor, $\alpha$. The adapter output is scaled by $\frac{\alpha}{r}$ before being added to the base output:

$$ h = W_0 x + \frac{\alpha}{r} (B A x) $$

The $\alpha$ parameter acts analogously to a learning rate multiplier for the LoRA modules. By decoupling the magnitude of the adapter activations from the rank $r$, it simplifies hyperparameter tuning. A standard heuristic dictates setting $\alpha = 2r$ or $\alpha = 1r$, depending on the target task complexity.
...
4.  **Adapter Path**: The LoRA adapter forward pass occurs simultaneously in full BF16: $h_2 = \frac{\alpha}{r} (B^{BF16} A^{BF16} x^{BF16})$.
...
file_path = r"d:\work\python-all\17-Generative-AI-and-LLMs\06_finetuning.md"
os.makedirs(os.path.dirname(file_path), exist_ok=True)

with open(file_path, "w", encoding="utf-8") as f:
    f.write(markdown_content)

print(f"Successfully wrote {len(markdown_content.split())} words to {file_path}")
