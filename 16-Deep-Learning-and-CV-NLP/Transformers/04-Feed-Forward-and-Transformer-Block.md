# The Transformer Block: Feed-Forward Networks, Residuals, and Layer Normalization
...
$$ 	ext{SwiGLU-FFN}(x_i) = \left( 	ext{Swish}(x_i W_1) \odot (x_i W_3) ight) W_2 $$
...
1.  **Gradient Superhighways:** During backpropagation, the derivative of the output with respect to the input $x$ is:
    $$ rac{\partial}{\partial x} (x + \mathcal{F}(x)) = 1 + \mathcal{F}'(x) $$
...
$$ \mu = rac{1}{d_{model}} \sum_{j=1}^{d_{model}} x_{i, j} $$
$$ \sigma^2 = rac{1}{d_{model}} \sum_{j=1}^{d_{model}} (x_{i, j} - \mu)^2 $$
...
$$ 	ext{LayerNorm}(x_i) = \gamma \odot rac{x_i - \mu}{\sqrt{\sigma^2 + \epsilon}} + eta $$
...
*   $eta \in \mathbb{R}^{d_{model}}$ is a learned shifting parameter (bias).
