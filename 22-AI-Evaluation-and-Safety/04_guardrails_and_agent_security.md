# Guardrails and Agent Security in Production
...
## 4. Human-in-the-Loop (HITL) Tool Execution Boundaries
...
docker run --rm   --network none   --memory 512m   --cpus 1.0   --security-opt="no-new-privileges:true"   --cap-drop=ALL   --read-only   -v /tmp/agent_scratchpad:/app/data:rw   python:3.11-slim python /app/agent_code.py
```
